...
- MUST 避開 5 大常見錯誤：vague / 缺 context / overload / 沒例子 / fragmented（破碎）
...
<file_map>
CLAUDE.md                  - 本檔（框架選用 + 6 品質維度 + 4 大錯誤）
...
templates/                 - 即填即用模板
  ...
  ├── rtf.md               - 簡單任務
  ├── rise-ix.md           - 創作 + 範例
  ├── universal-4-part.md  - 萬用模板（不確定用哪個就用這個）
  └── brain-dump.md        - 思緒不連貫時的鷹架（doing/observed/expected/actual/question）
</file_map>
...
| **Coherence** | 主詞清楚嗎？句子間有因果嗎？ | 跳結論、缺主詞、代詞指涉不明 | 邏輯線完整、自足可讀 |

> 任一維度 ≤2 分 → 直接跑 `/prompt-improve` 重寫
> 全部 ≥4 分 → 直接送
> **Coherence ≤2 時**：先用 [`templates/brain-dump.md`](./templates/brain-dump.md) 把 doing / observed / expected / actual / question 補齊，再跑 `/prompt-improve`
...
```text
我CD template有看到 nodeSelector 但我不知道為何都被指派到dnslocal=false的node
```

問題：缺主詞（哪個專案 / deployment？）、跳過 expected（你以為會排到哪？）、結論前推理鏈缺失。

✅ Good（用 brain-dump 模板還原跳過的步驟）

```markdown
<doing>我在排查 prod ns 的 deployment 為何 pod 沒排到 GPU node</doing>
<observed>deployment template 寫了 nodeSelector: env=prod</observed>
<expected>以為 pod 會排到 dnslocal=true 的 node（GPU 那批）</expected>
<actual>pod 全部跑到 dnslocal=false 的 node</actual>
<question>nodeSelector 之外，是什麼機制把 pod 推到 dnslocal=false？</question>
<tried>kubectl get deployment 確認 yaml，nodeSelector 確實是 env=prod</tried>
```

Why：腦中已經跑過好幾步推理才打字，文字常常只寫第 5 步。AI 不知道你跳過了什麼，會以「最統計平均」回應補洞，剛好補錯方向 → 答非所問。
**對策**：用 [`templates/brain-dump.md`](./templates/brain-dump.md) 強迫補主詞、補因果、補預期 vs 實際，再丟給 `/prompt-improve` 整理成連貫 prompt。
</paved_path>
...
- **AI 老是答不到點** → 對照 6 個品質維度自評，找出最低分那項補強
- **腦中很亂、寫出來跳來跳去** → 先填 [`templates/brain-dump.md`](./templates/brain-dump.md)（doing / observed / expected / actual / question），再丟 `/prompt-improve`
</common_tasks>
