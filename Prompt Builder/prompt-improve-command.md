...
3. **Read** `{{FRAMEWORK_PATH}}/Prompt Builder/CLAUDE.md` 與 `frameworks-cheatsheet.md` 取得框架知識
4. 開始 Step 1
</execution-plan>
...
| Coherence（連貫） | X/5 | <主詞缺漏 / 句子跳躍 / 缺因果> |

**總評**：<2 句話總結最弱的 1–2 項>
```

> 任一 ≤2 分 → 必須在 Step 2 釐清；全部 ≥4 分 → 直接 Step 4 微調即可。
> **Coherence ≤2 時優先處理**：先補主詞 / 因果再談其他維度，否則重寫會建立在錯誤前提上。

### Coherence 評分指引（破碎敘述偵測）

幫使用者打 Coherence 分數時，檢查以下信號：

- **主詞 / 受詞缺失**：「會出現 X 的可能?」沒主詞 / 沒情境
- **跳結論**：直接寫 actual 沒寫 expected（「為何都被指派到 X」沒講原本以為會怎樣）
- **代詞指涉不明**：「它」「這個」「那邊」找不到指涉對象
- **主題漂移**：一段內跳兩個以上不相關話題
- **隱含前提**：用了專案 / 工具特定詞彙但沒給出處（哪個 repo / 哪個 file / 哪個 deployment）
- **typo / 斷句錯**：本身不扣分，但配合上述信號代表草稿是腦中直譯

評分對照：

- 5：主詞清楚、有因果連接、context 完整自足
- 3：能讀懂但要猜 1–2 處
- 1：必須反問才能繼續，否則重寫會錯
</step-1-diagnose>
...
- **Coherence 弱**（≤2 分時優先）→ 反問還原跳過的步驟：
  - 「『XXX』的主詞是？是哪個專案 / 檔案 / 服務？」
  - 「你原本預期會怎樣？實際發生什麼？兩者落差在哪？」
  - 「『它 / 這個』指的是上面哪一段？」
  - 「你在想 A 還是 B？兩者你想先解哪個？」
  - 必要時建議使用者改用 `templates/brain-dump.md` 重寫草稿再來

**MUST 一次問完所有問題**（編號），**NEVER 拆多輪**。
**MUST Coherence ≤2 時，至少 2 題用來補主詞 / 因果**，其他維度的問題往後排。
...
- MUST 避開 5 大錯誤：vague / 缺 context / overload / 沒例子 / fragmented（破碎）
